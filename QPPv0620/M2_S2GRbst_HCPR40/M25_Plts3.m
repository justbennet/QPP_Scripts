...
for ig=1:nrgn
    ...
    if ie==1, a=QPPv_tm(ip,isd); 
        plot([a a],[0 hmx],'color',clh(a,:),'linewidth',1); end
    axis([1 PL 0 hmx]); xticks(5:5:PL); yticks([]); box on; 
    if ig~=nrgn, xticklabels([]); end
...
for ip=1:nP
    ...
    saveas(gcf,[d2plt '32_3_tp2_pval_SD' num2str(isd) '.png']); close
end
...
for isd=1:nsd(ip)
    ...
    a=ceil(max(abs(m(:)))*100)/100; ytk=[-a 0 a]; yticks(ytk);
    for i=xtk, plot([i i],[-a a],'k'); end
    axis([1 PLe -a a]); box on; 
