...
p2u='../../Utils/'; addpath(genpath(p2u)); % pth2 cifti/nii utilities
...
nP=3; % number of QPPs to detect
cth=cell(nP,1); % correlation threshold in the main algorithm
cth(1:min(nP,3))={[0.2 0.3]}; cth(4:end)={[0.1 0.2]};
ncth1=2*ones(nP,1); if nP>=4, ncth1(4:end)=3; end % #iters with lower cth
...
ITPstp=500; % step to show progress of algorithm when running ITP times
...
nnll=50; % number of null patterns for significance testing of QPP activity
...
%%%% Thresholds below were set after #segments/QPP is found (see SA2)
pth=0.1; % root sum square thresh. to include (vertices/)voxels within QPP
avth=0.025; % magnitude thresh. for (de/)activation of voxels within QPP
ayth=0.006; % mag. thresh. for (de/)activation of Yeo's RSN within QPP
apth=0.012; % mag. thresh. for (de/)activation of cortical parcels wthn QPP
